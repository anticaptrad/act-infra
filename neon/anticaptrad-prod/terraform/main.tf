...
resource "neon_project" "prod" {
  name      = "anticaptrad-prod"
  region_id = "aws-us-east-1"
}
resource "neon_role" "app"  { project_id = neon_project.prod.id; branch_id = neon_project.prod.default_branch_id; name = "anticaptrad_app" }
resource "neon_role" "auth" { project_id = neon_project.prod.id; branch_id = neon_project.prod.default_branch_id; name = "anticaptrad_auth" }
